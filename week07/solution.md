...
Every value except 0 is treated the same as “non-zero”.
So we only need:
non_zero_count = number of elements != 0zero_count = N - non_zero_count
answer = max(non_zero_count, zero_count)
________________________________________
🧠 How to Think / Problem Intuition
	You don’t care about the actual values except whether they are zero or non-zero
→ A major simplification.
	Therefore:
	If you encounter 0, increment zero counter.
	Otherwise, increment non-zero counter.
	The result is simply the larger group.
________________________________________
🚀 Solution Approach
Step-by-step:
